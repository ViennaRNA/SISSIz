...
TTree *NewTree()
{
	...
	tree->capacity=0;
	tree->names = NULL; /* fixed for CheckCapacity on empty tree->* */
	tree->tips  = NULL; /* (HAS ;-) */
	CheckCapacity(tree, 4000); /* from 1000  (TG)*/
...
{
  /* Stefan mod. */
  if (aNode->sequence != NULL) free(aNode->sequence);
	aNode->next=avail;
	avail=aNode;
} /* DisposeNode */
...
void FreeTree(TTree *tree)
{
  int i;
    ...
    for (i=0;i<tree->numTips;i++){
      if (tree->names[i]!=NULL) free(tree->names[i]);
    }
		free(tree->names);
    free(tree->tips);
...
void ReadUntil(FILE *fv, char stopChar, char *what)
{
	...
	if (feof(fv) || ch!=stopChar) {
		sprintf(treeErrorMsg, "%s missing2", what);
		treeError=1;
	}
}
...
TNode *ReadBranch(FILE *fv, TTree *tree, int numNames, char **names)
{
	...
	if (ch==':') {
		...
		if (tree->lengths==1) {
			//sprintf(treeErrorMsg, "Some branches don't have branch lengths");
			//return NULL;
      /* Workaround for the strange rooted tree of clearcut with one branch missing */
      len=0.0001;
		} else 
			tree->lengths=0;
